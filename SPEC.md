...
| OQ-15 *(v1)* | Coloured rasterisation cost in glyphcss (FR-DOME-7): `useColors` writes one span per run of colour. | Could halve the drag rate on a phone. | Measure in the first dome task under the D-62 method; `interactiveDownscale` is the fallback. |
...
- Sky dome, second pass: no frame, fills its box, colour by meaning, orientation cues, more detail, live marker, Sun and Moon; the dome is the default view again (§4.9).
