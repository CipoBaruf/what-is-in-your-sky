/**
 * R61 (D-319): the hook answers a query, follows it across its edge in both
...
describe('useMediaQuery', () => {
  it('answers the query and follows it across its edge', () => {
    media = stubMatchMedia(1659);
    const { result } = renderHook(() => useMediaQuery('(min-width: 1660px)'));
  ...
  it('detaches its listener on unmount', () => {
    ...
    const { unmount } = renderHook(() => useMediaQuery('(min-width: 1660px)'));
  ...
  it('is false where there is no matchMedia at all', () => {
    ...
    expect(renderHook(() => useMediaQuery('(min-width: 1660px)')).result.current).toBe(false);
