...
/**
 * The line compares its own bounds with the clock (F-23), so the clock is a
 * fixture too: T0 is "now" and the fixture passes run from it.
 */
beforeEach(() => {
...
describe('ReadinessLine (R27: FR-OFF-4)', () => {
  ...
  it('F-23: a stored run whose passes have all ended names the gap instead of a date in the past', () => {
    // The run was computed a week ago and its last pass ended five days back; the forecast still reaches ahead.
    set(ready([pass('a', -120)], T0 - 7 * 24 * HOUR));
    show();
    expect(screen.getByTestId('readiness')).toHaveTextContent('Not ready offline: no passes stored yet.');
    expect(screen.getByTestId('readiness')).not.toHaveTextContent('2026-09');
  });

  it('F-21: a warm start says nothing while the elements and the forecast are still in the air', () => {
    // The stored run is on screen first by design (PLAN §7.5) — on its own that is not a verdict.
    set({ ...done([pass('a', 4)], T0 - HOUR), elements: { status: 'loading' }, weather: { observer, status: 'loading', snapshot: null, error: null } });
    show();
    expect(screen.queryByTestId('readiness')).toBeNull();
    // The forecast lands first: still nothing, because the elements have not answered.
    set({ weather: { observer, status: 'ready', snapshot, error: null } });
    expect(screen.queryByTestId('readiness')).toBeNull();
    set({ elements: withRecords });
    expect(screen.getByTestId('readiness')).toHaveTextContent('Ready offline until');
  });
