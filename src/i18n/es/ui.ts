...
/**
 * FR-I18N-2: the `ui` lane's own section of the Spanish catalog, typed
 * against `en/ui.ts`'s shape (D-199 (2)), so a key missing here is a
 * `tsc -b` failure and never a runtime fallback to English (D-69).
 *
...
export const ui: typeof EnUi = {
  ...
  location: {
    ...
    searchFailed: { before: 'Mientras tanto se pueden ', link: coordsInstead, after: '.' },
  ...
  home: {
    ...
    savedFoot: 'Guardada solo en este navegador. Sin cuenta, sin rastreo.',
    ghost: {
      whenHeading: '¿Cuándo está lo bastante oscuro?',
      whenSentence: 'La banda oscura del lugar, con las nubes y la Luna de esta noche.',
  ...
  passes: {
    ...
    faintToggle: (p) => `${p.shown ? 'ocultar' : 'ver'} ${String(p.count)} ${p.count === 1 ? 'tenue' : 'tenues'}`,
    faintTag: 'tenue',
    ...
    nights: {
      ...
      onlyFaint: (faint) => `0 pases · ${String(faint)} ${faint === 1 ? 'tenue' : 'tenues'}`,
  ...
  footer: {
    ...
    chart: { before: 'Carta del cielo: ', link: 'glyphcss', middle: ', de ', link2: 'Juan Cruz Fortunatti', after: '.' },
    short: {
      sources: 'Datos:',
      licence: '(CC BY 4.0)',
      privacy: 'Sin rastreo',
