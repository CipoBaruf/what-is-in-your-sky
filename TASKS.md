...
- [ ] **P3 — The documents made true, the public half named, the reading order carried to v2.1**
  ...
  - **Scope (SPEC §4.43, PLAN D-650, D-651):**
    ...
    - `README.md`'s glyphcss paragraph loses the author's name (FR-PUB-4 as amended, V22-15): "[glyphcss](https://glyphcss.com) — **MIT**, © 2025 Layoutit" and the line on why it was chosen; `tests/docs/public.test.ts`'s assertion on the name becomes one on the library, the URL and the copyright holder; `docs/RELEASE.md` §14's footer item names the library alone. After this, `git grep -i <the author's surname>` over the tracked tree returns only SPEC's Decision Log rows and history notes, PLAN D-657's history sentence and TASKS's checked P1 entry and this one.
