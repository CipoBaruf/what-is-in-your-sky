...
- [x] **P4 — The recording run: phone and desktop flows into `promo/`**
  - **Done (2026-09-25, D-671..D-673):** as written, with three things the tools decided. The `promo` project is in `playwright.config.ts` only when `--project=promo` is on the command line (a bare `playwright test` runs every project; the answer reaches the workers as `PLAYWRIGHT_PROMO=1`, D-671). Each flow opens its own context with its device's `recordVideo`, since Playwright refuses `test.use({ video })` inside a `describe` (D-672). The seeds module is `tests/e2e/captureSeeds.ts` and the list `tests/e2e/promoFlows.ts`, which the spec re-exports as `PROMO_FLOWS` (D-673). The frame produced: the phone's 1170 × 2532 as asked (VP8, 25 fps, no cap), the desk's 1440 × 900; the eight files are 17–29 s. "Light" in the flow list is read as the app's `night` theme. The phone's live flow is Paris by day (`CLOCK` + 12 h) so `[ see this pass ]` is on the headline. `ffmpeg` was on `PATH` for the headless run, so every `.webm` has its `.mp4`.
  - **Lane:** ui
  - **Model:** fable
  - **Review:** opus
  - **Turns:** 500
