...
- [x] **R61 — The wide live page's rail: the rows go beside the dome, and the dome fills the width** *(added 2026-09-07 as a hotfix on the owner's ask, out of wave order; PLAN §16.9 gives it D-312..D-316. Rewritten the same day on the owner's review of the preview, V12-8 and V12-9: the box is a ladder of fixed sizes, the stripe is under the box from step 2, and the resize defect F-60 is fixed with it.)*
  - **Lane:** live
  - **Model:** opus
  - **Gate:** owner
  - **Depends on:** none
  - **Findings:** F-59, F-60
  - **Goal:** On a desktop the dome box is one of four fixed sizes the drawing fills across and down, the rows that were under it stand beside it, the stripe is under it on a big screen, and a resized window draws the dome right.
  ...
  - **Scope, rewritten (PLAN D-314..D-316):** `lib/layout.ts` carries the ladder (`DOME_STEPS`: 768 × 544 from 1258 × 664, 1176 × 833 from 1666 × 1061, 1680 × 1190 from 2170 × 1418, 2688 × 1904 from 3178 × 2132, every step 2.4 : 1.7) and `hooks/useDomeStep.ts` reads it as one `matchMedia` query per step on width and height; `SkyChartProps` gains `box` and `stripe`, `ChartFrame` writes the box inline as `--chart-box-w` / `--chart-box-h` and gives the stripe a row of its own under the drawing; `Live.tsx` hands the stripe block to `stripe` from step 2 and keeps it in the rail at step 1; `SkyDome`'s layer keys carry the font size, so a resize remounts the scene (F-60).
  - **Done when:**
    - `live-rail.spec.ts` at 1280 × 800, 1280 × 720, 1920 × 1080, 2560 × 1440 and 3840 × 2160: the box is its step's size, the strip, the playback row and the share action stand beside it, the stripe block is in the rail at step 1 and under the box at the box's width from step 2, the page does not scroll, and the painted drawing covers at least 90 % of the box's **width** — **failing on the old layout** everywhere (65 %, 66 %, 68 %, 69 %).
    ...
    - `live.spec.ts`'s wide block asserts the rail's order, the stripe's place at each step and that no stripe label is drawn over another; `Live.test.tsx`, `useDomeStep.test.ts`, `domeSteps.test.ts` and `SkyChart.contract.test.tsx` assert the step, the placement, the ladder's arithmetic and the stylesheet's rules.
  ...
  - **Done 2026-09-07:** first as the rail alone (D-312, D-313), then rewritten the same day on the owner's review of the preview (V12-8, V12-9; D-314..D-316). Measured on the ladder, box then drawing across: 1280 × 800 → 768 × 544, 92 %; 1920 × 1080 → 1176 × 833, 92 %; 2560 × 1440 → 1680 × 1190, 92 %; 3840 × 2160 → 2688 × 1904, 92 % (from 65–69 % on the old layout). The four screenshots the owner sent were one tab resized, and the resize was F-60: the scene's remount key carried the column count alone, which every desktop box caps; with the font size in the key the resize chain gives the fresh-load numbers exactly. One recorded lane crossing (the `aside`, `stripe` and `box` slots in `chart`). Captures: `docs/screenshots/r61-live-{1280,1920,2560,3840}-dark-en.png` and `r61-live-1280-night-en.png`; the v1 set's live and legend captures at 1280 px re-shot, and R54's fold captures left as the record of the layout this replaces.
