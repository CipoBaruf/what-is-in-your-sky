...
- [x] **R61 — The wide live page's rail: the rows go beside the dome, and the dome fills the width** *(added 2026-09-07 as a hotfix on the owner's ask, out of wave order; PLAN §16.9 gives it D-312..D-316. Rewritten the same day on the owner's review of the previews, V12-8..V12-10: the box is cut to the drawing's shape from what the window leaves, the stripe is under the box from 1666 px, and the resize defect F-60 is fixed with it.)*
  ...
  - **Depends on:** none
  - **Findings:** F-59, F-60
  - **Goal:** On a desktop the dome box is the largest box of the drawing's own shape the window leaves, filled across and down, the rows that were under it stand beside it, the stripe is under it on a big screen, and a resized window draws the dome right.
  ...
  - **Done 2026-09-07:** first as the rail alone (D-312, D-313), then rewritten the same day on the owner's review of the previews (V12-8..V12-10; D-314..D-316) — a ladder of four fixed boxes was tried and withdrawn within the day, because a browser window is never its screen (a 2560 × 1440 monitor's 1235 px viewport took the 1920 step). Measured on the cut box, box then drawing across: 1280 × 800 → 798 × 565, width-bound beside the rail, 92 %; 1920 × 1080 → 1202 × 852 through the stripe under it, 92 %; 2560 × 1235 → 1421 × 1007, 92 %; 3840 × 2160 → 2727 × 1932, 92 % (from 65–69 % on the old layout). The four screenshots the owner sent first were one tab resized, and the resize was F-60: the scene's remount key carried the column count alone, which every desktop box caps; with the font size in the key the resize chain gives the fresh-load numbers exactly. One recorded lane crossing (the `aside`, `stripe` and `boxAspect` slots in `chart`). Captures: `docs/screenshots/r61-live-{1280,1920,2560,3840}-dark-en.png` and `r61-live-1280-night-en.png`; the v1 set's live and legend captures at 1280 px re-shot, and R54's fold captures left as the record of the layout this replaces.
