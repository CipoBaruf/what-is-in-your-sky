...
3. Confirm the working tree is clean. Interactive: fetch, then create a branch `r<n>-<short-slug>` from `origin/main` before the first edit. Headless: the driver has already made a worktree of its own for the task and a branch off `origin/main` in it (PLAN §16.4 step 2), so stay on that branch. Either way, never commit to `main`, and never `git add -A` — name the files, so nothing that happens to be in the tree rides along.
...
4. Check the task off in TASKS.md on this branch only, adding a one-line note if what was done differs from the task text. Merging to `main` is what marks it done for the project.
