...
// Four projects: jsdom for `src/ui`, Node for everything else, (R5) a
// Chromium browser project for the worker integration test, which boots the
// real module worker (PLAN §9.1), and (R11) `live`, the `LIVE=1` contract
// suite in `tests/live` with no MSW at all — it is the one place tests reach
// the network, and it skips itself without `LIVE=1`. Node and jsdom start the
// MSW server so no unit test can reach the network (PLAN §9.3); the browser
// project makes no requests at all (fixtures are imported as modules).
    ...
    projects: [
      {
        ...
        test: {
          ...
          exclude: ['src/ui/**', 'src/**/*.integration.test.ts', 'tests/e2e/**', 'tests/live/**', 'node_modules/**'],
