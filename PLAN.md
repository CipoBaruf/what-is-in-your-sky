...
- **Fixture:** `tests/fixtures/open-meteo/2026-09-02-rosario-geocode.json` is the verbatim response to the §7.2 URL for `name=rosario`, captured 2026-09-02T20:03:55Z with `access-control-allow-origin: *` confirmed: eight results across five countries, an ambiguous pick list by design. MSW serves it for that name, the provider's no-match shape for any other, and 400 for a request that is not the §7.2 one.
- **Playwright:** `place-search.spec.ts` runs at 390 px, types "Rosario" key by key and expects one request, checks every row (name, region, height ≥ 44 px, inside the viewport, font ≥ 16 px), picks the first row, and expects the confirmation and a pass list whose times are already `GMT-3` with the forecast route aborted — the zone came from the geocoding result (FR-LOC-3). Screenshot in `docs/screenshots/r9-place-picker-390.png`.
