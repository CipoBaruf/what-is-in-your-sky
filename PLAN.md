...
│   │   │   ├── now/ NowPanel.tsx
│   │   │   ├── passes/ PassList.tsx  PassCard.tsx  IssHeroCard.tsx  SortToggle.tsx  nightGroups.ts (the run's window -> one group per night, D-146)
...
│   │   ├── hooks/ useNow.ts (the wall clock, shared from R11)  useLayoutMode.ts (compact | wide over matchMedia, D-72/D-116)  useOnline.ts (navigator.onLine, FR-OFF-8, D-147)
