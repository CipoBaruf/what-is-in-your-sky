...
- R7 (`r7-now-panel`, worked in parallel) also adds `jest-axe` and touches `App.tsx` and `tests/setup/vitest.jsdom.ts`; whichever branch merges second resolves those three files.
...
│   ├── ui/
│   │   ├── App.tsx
│   │   ├── screens/ Home.tsx  PassDetail.tsx  passSelection.ts (hash ↔ selected pass, D-13/D-31)
│   │   ├── components/
