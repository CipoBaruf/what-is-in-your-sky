...
- **D-112 — FR-DOME-1's finer desktop drawing is capped by the shared chart box, not by the column cap.** `ChartFrame` sizes every view with `--chart-size: calc(44 * var(--cell))` ≈ 422 px (R13/R15, so the dome and the polar chart are the same size and the view toggle moves nothing). At a 1280 px viewport the dome therefore fills a 422 px box and takes **65 columns**, not the 120 D-91 allows: the drawing does fill its container, but the container is the thing that stays phone-sized. Widening it is a change to the shared frame and so to the polar view, which R21 is explicitly not allowed to touch, so it is left as a follow-up for the task that owns the guide's desktop layout. The D-91 cap and its measurement stand for whenever the box does grow.
